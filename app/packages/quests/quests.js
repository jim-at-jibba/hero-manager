...
export const QuestsCollection = new Mongo.Collection('quests');
...
const generateQuest = (userId, act = 1) => {
  ...
  let monsterList = R.range(1, U.num(2, 3)).map(() =>
    fillMonsterParty(act, U.num(1, 4), zone)).sort((a, b) => a.speed < b.speed);

  let difficultyRating = Math.ceil(R.flatten(monsterList).reduce((x, y) => {
      return x + y.health * (y.power * 0.75);
    },
    0
  ));
  let reward = Math.ceil((difficultyRating * 0.75 + U.num(0, 50)) * (1 + act / 10));
...
const fillQuestList = (userId, act = 1, list = [], length = 10) => {
  return list.length === length ?
    list :
    fillQuestList(userId, act, list.concat(generateQuest(userId, act)), length);
};

const addNew = (userId, act = 1) => {
  QuestsCollection.insert(
    generateQuest(userId, act)
  );
};
  ...
  return () => {
    ...
    if (_count >= 40) {
      ...
      users.forEach((user) => {
        ...
        while (act < 3) {
          act++;
          let len = QuestsCollection.find({userId: user._id, act}).fetch().length;
          if (len < 10) {
            addNew(user._id, act);
          }
        }
